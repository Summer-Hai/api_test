...
class BaseApi():
    ...
    # 通过配置文件获取测试的环境的ip地址
    ip = cf.get_key("dev_env", "formal_ip")
    username = cf.get_key('dev_env', 'username')
    password = cf.get_key('dev_env', 'password')
    ...
    def get_token(self):
        ...
        """
        log.info("获取用户token")
        # ip = cf.get_key('formal_ip')
        corpid = cf.get_key('test_env', 'corpid')
        corpsecret = cf.get_key('test_env', 'corpsecret')
        data = {
            'method': 'GET',
            'url': f'https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid={corpid}&corpsecret={corpsecret}'
        }
        access_token = self.send_api(data)['access_token']
...
if __name__ == "__main__":
    b = BaseApi()
    # user = b.load_yaml("datas/token.yml", "user1")
    # update = f"UPDATE `python14`.`ad_agent` SET `name` = '测试', `full_name` = '白云深处有人家', `contact_person` = NULL, `contact_number` = NULL, `company_pay_id` = 1, `type` = 1, `custody_rate` = {user['age']} WHERE `id` = 24;"
    # sql.update(update)
    # data = b.load_yaml('datas/MarketBasicN_data.yml', 'data')
    # print(data)
    # print(user)
    print(b.get_token())
